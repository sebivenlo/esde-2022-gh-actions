/*
 * Copyright (c) 2022 - for information on the respective copyright owner
 * see the NOTICE file and/or the repository https://github.com/grevend/shamirs-secret-sharing.
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include "secret_sharing.h"

#ifdef __x86_64__
int _arch_rdrand_support() {
    static int supported = -1;
    unsigned int ecx, other;
    if(supported == -1) {
        asm volatile("cpuid" 
            : "=a" (*(&other)), "=b" (*(&other)), "=c" (*(&ecx)), "=d" (*(&other)) 
            : "a" (1), "c" (0)
        );
        supported = ecx & 0x40000000 == 0x40000000;
    }
    return supported;
}
#endif

extern inline char *_rand_bytes_economical(size_t len) {
    const size_t iterations = NEAREST(len, sizeof(int));
    int *arr = malloc(iterations);
    for (size_t i = 0; i < iterations / sizeof(int); i++)
        arr[i] = rand();
    return (char*) arr;
}

#ifdef __x86_64__
extern inline char *_rand_bytes_real_random(size_t len) {
    const size_t iterations = NEAREST(len, sizeof(uint64_t));
    uint64_t *arr = malloc(iterations);
    for (size_t i = 0; i < iterations / sizeof(uint64_t); i++) {
        unsigned int attempts = 10;
        do {
            unsigned char ok;
            uint64_t rand = 0;
            asm volatile ("rdrand %0; setc %1" 
                : "=r" (*(&rand)), "=qm" (ok));
            if(ok) {
                arr[i] = rand;
                break;
            }
        } while(--attempts);
    }
    return (char*) arr;
}
#endif

extern inline char *_rand_bytes(size_t len) {
#ifdef __x86_64__
    if(_arch_rdrand_support()) return _rand_bytes_real_random(len);
#endif
    return _rand_bytes_economical(len);
}

const char _LOG[] = {
    ,
};

const char _EXP[] = {
    ,
};

extern inline char _mul(char l, char r) {
    if(l == 0 || r == 0) return 0;
    return _EXP[UINT(_LOG[UINT(l)]) + UINT(_LOG[UINT(r)])];
}

extern inline char _div(char l, char r) {
    return _mul(l, UINT(_EXP[255 - UINT(_LOG[UINT(r)])]));
}

unsigned int _degree(size_t polyc, char *polyv) {
    for(size_t i = polyc - 1; i >= 1; i--)
        if(polyv[i] != 0) return i;
    return 0;
}

char *_poly(rand_gen gen, int degree, char x) {
    char *poly = gen(degree + 1);
    while(_degree(degree + 1, poly) != degree)
        poly = gen(degree + 1);
    poly[0] = x;
    return poly;
}

char _eval(size_t polyc, char *polyv, char x) {
    char res = 0;
    for(int i = polyc - 1; i >= 0; i--)
        res = ADD(_mul(res, x), polyv[i]);
    return res;
}

char *shares(size_t secretc, char *secretv, rand_gen gen, size_t n, size_t k) {
    char *values = malloc(n * (secretc + 1));
    for(size_t i = 0; i < secretc; i++) {
        char *poly = _poly(gen, k - 1, secretv[i]);
        for(size_t x = 1; x <= n; x++) {
            if(i == 0) values[(secretc + 1) * (x - 1)] = x;
            values[(secretc + 1) * (x - 1) + i + 1] = _eval(k, poly, x);
        }
        free(poly);
    }
    return values;
}

char _interpolate(size_t pointc, char *pointsv) {
    char y = 0, t = 1;
    for(size_t i = 0; i < pointc; i++, t = 1) {
        const char iX = pointsv[i * 2];
        const char iY = pointsv[(i * 2) + 1];
        for(size_t j = 0; j < pointc; j++) {
            const char jX = pointsv[j * 2];
            if(i != j) t = _mul(t, _div(SUB(0, jX), SUB(iX, jX)));
        }
        y = ADD(y, _mul(t, iY));
    }
    return y;
}

char *reconstruct(size_t partc, size_t polyc, char *partv) {
    char *secret = malloc(polyc);
    char *points = malloc(partc * 2);
    for(size_t i = 1; i < polyc; i++) {
        for(size_t j = 0; j < partc; j++) {
            points[j * 2] = partv[polyc * j];
            points[j * 2 + 1] = partv[polyc * j + i];
        }
        secret[i - 1] = _interpolate(partc, points);
    }
    free(points);
    return secret;
}
